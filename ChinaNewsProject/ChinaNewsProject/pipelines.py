...
class ChinanewsprojectPipeline(object):
    def __init__(self):
        ...
        # db_config = {"host":'172.18.113.108', "port":3306, "user":'Dev3', "passwd":'Dev3', "db":'LYNews', "charset":'utf8'}
        # db_config = {"host": 'localhost', "port": 3306, "user": 'root', "passwd": '123456', "db": 'sys', "charset": 'utf8'}
        print('链接数据库成功')
    ...
    def process_item(self, item, spider):
            ...
            if len(value) >= 0:
                ...
                if NewsTitles not in value:
                    ...
                    if item['NewsTitle'] == '' or item['NewsContent'] == '':
                        ...
                        self.red.lpush('2018-05-pynews' + item['NewsCategory'], item)
                else:
                    print('redis数据已存在')
            else:
                print('出错')
        ...
        # 数据库去重
        self.cursor.execute("SELECT NewsTitle FROM tbl_NewsDetails WHERE NewsTitle='%s'" % item['NewsTitle'])
