...
class MysqlTwistedPipeline(object):
    def __init__(self):
        ...
        db_config = {"host": '172.18.115.15', "port": 3306, "user": 'Dev3', "passwd": '123456', "db": 'Im',
                     "charset": 'utf8'}
        # db_config = {"host": ' 10.100.105.133', "port": 3316, "user": 'Dev3', "passwd": '123456', "db": 'Im',
        #              "charset": 'utf8'}
    ...
    def process_item(self, item, spider):
        try:
            ...
            if len(value) >= 0:
                ...
                if NewsTitles not in value:
                    ...
                    if item['NewsTitle'] == '':
                        ...
                        self.red.lpush('201806news' + item['NewsCategory'], items)
                else:
                    print('redis数据已存在')
            else:
                print('出错')
            ...
            print('错误操作')
        # 数据库去重
        self.cur.execute("SELECT NewsTitle FROM tbl_NewsDetails201806 WHERE NewsTitle='%s'" % item['NewsTitle'])
        ...
        if cam_rows is not None:
            ...
            sql1 = 'insert ignore into tbl_NewsDetails201806(NewsID, NewsCategory, SourceCategory, NewsType, NewsTitle, NewsRawUrl, SourceName, AuthorName, InsertDate, NewsContent, NewsDate, NewsClickLike, NewsBad, NewsRead, NewsOffline)' \
                   'VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            sql2 = 'insert into tbl_NewsFileManager201806(FileID, FileType, FileDirectory, FileDirectoryCompress, FileDate, FileLength, FileUserID, Description, NewsID,image_url)' \
                   'VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            try:
                if item['NewsTitle'] == '':
                    self.cur.execute('DELETE FROM tbl_NewsDetails201806 WHERE NewsID=%s' % item["NewsID"])
                else:
...
#         db_config = {"host": '172.18.115.15', "port": 3306, "user": 'Dev3', "passwd": '123456', "db": 'Im',
#                      "charset": 'utf8'}
#         # db_config = {"host": '10.100.105.142', "port": 3306, "user": 'Dev3', "passwd": 'Dev3', "db": 'Im',
#         #              "charset": 'utf8'}
#         self.pool = PooledDB(pymysql, 5, **db_config)
#         # 5为连接池里的最少连接数
#
