...
class NewsPipeline(object):
    def __init__(self):
        # redis_nodes = [
        #                {'host': '172.18.113.100', 'port': 6379},
        #                ]
        #
        # self.red = StrictRedisCluster(startup_nodes=redis_nodes)
        self.red = redis.Redis(host="10.100.102.149", port=6379, db=2, decode_responses=True)
        print('连接成功')
        print('连接mysql服务器')
    ...
    def process_item(self, item, spider):
        try:
            ...
            # 转变类型为字符串
            key = ''.join('%s' % key for key in keys)
