...
path = '/home/user/桌面/images/full'

class HuanqiuSpider(CrawlSpider):
    name = 'GuoJi'
    # allowed_domains = ['www.huanqiu.com']
    start_urls = [
        'http://world.huanqiu.com/article/',  # 国际
    ]

    rules = (
        Rule(LinkExtractor(allow=r'http://world.huanqiu.com/article/\w+\.html'), callback='parse_item', follow=True),
    )

    def parse_item(self, response):
        # print(response.url)
        # print('==================')
        div_list = response.xpath('//div/ul/li[starts-with(@class,"item")]')
        for div in div_list:
    ...
    def parse_info(self, response):
        ...
        item['AuthorName'] = response.xpath(
            ...
            '//div[@class="editorSign"]/span[@id="editor_baidu"]/text()').extract_first()[3:6]

        item['SourceCategory'] = '环球国际滚动新闻'
        item['NewsType'] = 0
        ...
        if image_urls:
            ...
            item['NewsContent'] = content = ''.join(
                response.xpath('//div[@class="la_con"]/p | //div[@id="text"]/p | //div[@class="la_con"]/p | //div[@class="m_l"]//a/img').extract())
