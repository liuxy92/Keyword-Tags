...
class MysqlTwistedPipeline(object):
    ...
    def process_item(self, item, spider):
        try:
            ...
            print('错误操作')
        # 数据库去重
        self.cur.execute("SELECT NewsTitle FROM tbl_NewsDetails201804 WHERE NewsTitle='%s'" % item['NewsTitle'])
        ...
        if cam_rows is not None:
            ...
            sql1 = 'insert ignore into tbl_NewsDetails201804(NewsID, NewsCategory, SourceCategory, NewsType, NewsTitle, NewsRawUrl, SourceName, AuthorName, InsertDate, NewsContent, NewsDate, NewsClickLike, NewsBad, NewsRead, NewsOffline)' \
            ...
            try:
                if item['NewsTitle'] == '' or item['NewsContent'] == '':
                    self.cur.execute('DELETE FROM tbl_NewsDetails201804 WHERE NewsID=%s' % item["NewsID"])
                else:
