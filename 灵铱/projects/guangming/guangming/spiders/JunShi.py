...
class JunshiSpider(CrawlSpider):
    ...
    start_urls = ['http://mil.gmw.cn/']

    rules = (
        # Rule(LinkExtractor(allow=r'http://mil.gmw.cn/node_\d+\.htm')),
        Rule(LinkExtractor(allow=r'http://mil.gmw.cn/node_\d+\w+\.htm'), callback='parse_item', follow=True),
    ...
    def parse_item(self, response):
        ...
        for div in div_list:
            ...
            urls = 'http://mil.gmw.cn/' + str(link)

            yield scrapy.Request(url=urls, callback=self.parse_info, meta={'item': item})


        # div_list = response.xpath('//div[@class="channelMain"]/div[@class="channelLeftPart"]')
        # for div in div_list:
        #     item = GuangmingItem()
        #     item['NewsID'] = str(uuid.uuid1())
        #     item['NewsCategory'] = '001.011'
        ...
        #     item['NewsType'] = 0
        #     item['NewsTitle'] = div.xpath('./div/ul[@class="channel-newsGroup"]/li/a/text()').extract_first()
        #     item['NewsDate'] = div.xpath('./div/ul[@class="channel-newsGroup"]/li/span/text()').extract_first()
        #     link = div.xpath('./div/ul[@class="channel-newsGroup"]/li/a/@href').extract_first()
        #     urls = 'http://mil.gmw.cn/' + str(link)
        #
        #     yield scrapy.Request(url=urls, callback=self.parse_info, meta={'item': item})
